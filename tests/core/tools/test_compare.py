...
def test_choose_by_key_old_new_triple():
    ...
    result = CompareRules.get_comparator(
        rules, DefaultCmp, key="size", old=int, new=float
    )
...
def test_choose_by_key_only():
    ...
    result = CompareRules.get_comparator(
        rules, DefaultCmp, key="size", old=str, new=str
    )
...
def test_choose_by_type_pair():
    ...
    result = CompareRules.get_comparator(
        rules, DefaultCmp, key="other", old=int, new=float
    )
...
def test_fallback_old_is_NoneType_uses_new_type_rule():
    ...
    result = CompareRules.get_comparator(
        rules, DefaultCmp, key="x", old=NoneType, new=str
    )
...
def test_fallback_old_not_NoneType_uses_old_type_rule():
    ...
    result = CompareRules.get_comparator(
        rules, DefaultCmp, key="y", old=float, new=int
    )
...
def test_no_rule_returns_default():
    result = CompareRules.get_comparator(
        rules={}, default=DefaultCmp, key="k", old=int, new=int
    )
