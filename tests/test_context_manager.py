...
class TestContextManager:
    ...
    def test_add_context_information_multiple_context_keys(self, mock_get_context):
        ...
        old_schema = {
            "properties": {"name": {"type": "string", "format": "email", "minimum": 0}}
        }
        new_schema = {
            "properties": {"name": {"type": "integer", "format": "email", "minimum": 0}}
        }
        manager = ContextManager(old_schema, new_schema)
    ...
    def test_find_context_value_from_old_schema(self):
        ...
        with patch(
            "jsonschema_diff.context_manager.PathUtils.get_value_at_path"
        ) as mock_path:
            mock_path.side_effect = lambda schema, path: (
                "email" if "email" in str(schema) else None
            )
    ...
    def test_add_context_information_operation_filtering(
        self, mock_operation, mock_get_context
    ):
