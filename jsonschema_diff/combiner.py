...
class ParameterCombiner:
            ...
            for rule in combination_rules:
                ...
                if len(path) > 0 and path[-1] == main_param:
                    ...
                    if self._can_combine(rule, path, old_val, new_val, "main"):
                        ...
                        sub_diff = self._find_or_create_sub_param(differences, sub_path, rule, "main")
                        if sub_diff:
                            combined_diff = self._combine_params(rule, (path, old_val, new_val), sub_diff)
                    ...
                    if self._can_combine(rule, path, old_val, new_val, "sub"):
                        ...
                        main_diff = self._find_or_create_main_param(differences, main_path, rule, "sub")
                        if main_diff:
                            combined_diff = self._combine_params(rule, main_diff, (path, old_val, new_val))
                            result.append(combined_diff)
                            processed_paths.add(path_str)
    ...
    def _get_schema_value(self, param_path: List[str], schema: Dict[str, Any]) -> Optional[Any]:
